-- Serialize first-time workspace creation per authenticated user.
-- Without this lock, two app instances can both observe no owner and create
-- separate workspaces for the same master.
...
begin
  ...
  if p_duration_minutes <= 0 or p_price_amount < 0 then
...
revoke all on function public.bootstrap_master_workspace(text, text, text, text, integer, numeric, time, time) from public;
grant execute on function public.bootstrap_master_workspace(text, text, text, text, integer, numeric, time, time) to authenticated;
